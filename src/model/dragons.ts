...
export function addDragon(name:string,rider:number,avatar:number) {
    return new Promise((result, rej) => {
        db.query(`INSERT INTO dragon (id, name, level, attack, defense, slip,rider,avatar) VALUES (NULL, ?, 1, 3, 2, 1,?,?);`, [name,rider,avatar]).then((data:any) => {
            const retObj = data
            result(retObj)
        }).catch((err:Error) => {
            rej(err)
...
export function getAvatar(){
    return new Promise ((result,rej) => {
        db.query(`SELECT * FROM avatar`,[]).then((data:any) => {
            const retObj = data
            result(retObj)
        }).catch((err:Error) => {
            rej(err)
